...
export function AddWorkerDialog({ onWorkerAdded }: { onWorkerAdded: () => void }) {
    ...
    const handleClose = () => {
        ...
        setFormData({ name: "", ip: "", port: "3000", apiKey: "" });
        setCopied(false);
        setError(null);
    };
                ...
                <DialogContent className="sm:max-w-[500px] border-zinc-800 bg-zinc-950">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-zinc-400">
                            Copie cette clé et mets-la dans le fichier <code>config.json</code> du worker, champ <code>sharedKey</code>. C&apos;est le seul champ requis.
                            <br />
                ...
                <DialogContent className="sm:max-w-[425px]">
                    ...
                    <form onSubmit={handleSubmit} className="grid gap-4 py-4">
                        ...
                        <div className="space-y-2">
                            ...
                            {showAdvanced && (
                                <div className="grid gap-2">
                                    <Label htmlFor="apiKey">Clé existante (optionnel)</Label>
                                    <Input
                                        id="apiKey"
                                        value={formData.apiKey}
                                        onChange={(e) => setFormData({ ...formData, apiKey: e.target.value })}
                                        placeholder="Laisser vide pour générer et pousser automatiquement"
                                    />
                                    <p className="text-xs text-muted-foreground">
                                        Renseigne cette clé si le worker en possède déjà une dans son config.json. Sinon laisse vide pour générer une nouvelle clé à copier manuellement.
                                    </p>
                                </div>
                            )}
                        </div>
